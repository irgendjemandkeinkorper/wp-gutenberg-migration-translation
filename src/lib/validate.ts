...
function enforceWhitelist(body: HTMLElement): void {
  ...
  const els1 = body.querySelectorAll("*");
  for (let i = 0; i < els1.length; i++) {
    const el = els1[i] as Element;
  ...
  const els2 = body.querySelectorAll("*");
  for (let i = 0; i < els2.length; i++) {
    const el = els2[i] as Element;
...
function isolateTokens(body: HTMLElement): void {
  ...
  while (child) {
    ...
    // ⚡ Bolt: Avoid Array.from allocation; use native firstChild/nextSibling
  ...
  while (child2) {
    const nextChild2 = child2.nextElementSibling;
