...
function safeAttributes(node: SemanticNode, allowed: ReadonlySet<string>, findings: CompilerFinding[]): string {
  ...
  const validAttributes: [string, string][] = [];
  ...
  for (const [name, value] of attributes) {
    ...
    if (lowerName === "href" && !isSafeUrl(String(value))) {
      findings.push({
        code: "unsafe-inline-url",
        message: `Unsafe URL stripped from ${name} attribute.`,
        severity: "warning",
        sourceNodeId: node.id,
      });
      continue;
    ...
    validAttributes.push([lowerName, String(value)]);
  }
