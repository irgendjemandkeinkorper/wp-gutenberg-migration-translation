...
import { buildSourceEvidencePackage, readSourceEvidencePackage, snapshotsFromSourceEvidencePackage, writeSourceEvidencePackage } from "../lib/acquisition/source-package";
...
function snapshot(id: string, url: string): ArchivedPageSnapshot {
  return {
    record: {
      contractVersion: "1.0.0", semanticVersion: "1.0.0", recordId: id, recordKind: "page-snapshot", outcome: "success", requestedUrl: url, finalUrl: url, redirectChain: [],
      retrieval: { retrievedAt: "2026-08-03T00:00:00.000Z", method: "GET", userAgent: "fixture", durationMs: 1, responseHeaders: { authorization: "secret", "content-type": "text/html" } },
      status: 200, encoding: { declared: "utf-8", used: "utf-8", source: "content-type" }, discovery: { parentUrl: null, depth: 0 }, policy: { decision: "allow", reason: "fixture", robots: "allowed" }, errors: [],
      content: { rawBytes: { sha256: "raw", byteLength: 5, storageKey: "raw/a" }, decodedHtml: { sha256: "html", byteLength: 14, storageKey: "html/a" } }, compatibility: { reader: "forward-compatible", minimumReaderVersion: "1.0.0", unknownFields: "ignore" },
    }, decodedHtml: `<p>${id}</p>`,
  };
}
...
describe("portable source-evidence package", () => {
  it("exports redacted records, stable duplicate-safe files, and reconstructs offline snapshots", async () => {
    const source = buildSourceEvidencePackage({ snapshots: [snapshot("page/one", "https://example.test/a"), snapshot("page-two", "https://example.test/a")], createdAt: "2026-08-03T00:00:00.000Z" });
