...
## 2024-11-21 - [Extracting list items in React to avoid O(N) re-renders]
**Learning:** Extracting list items connected to frequently updated Map state (like `batchStatus` in `SourceInputPanel`) into a `React.memo` component significantly improves performance. The memoized list item component avoids re-rendering when other Map entries change because its specific `batchStatus.get(index)` prop remains stable.
**Action:** To prevent O(N) re-renders in large lists, wrap the rendered item inside a `React.memo` component and pass the specific Map value (e.g., `batchStatus.get(index)`) as a prop.
