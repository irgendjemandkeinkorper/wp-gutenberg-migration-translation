...
## 2024-11-21 - [O(N) List Re-renders on Frequent Updates]
**Learning:** Rendering a list of objects derived from a Map (like `batchStatus` in `SourceInputPanel`) where the parent re-renders frequently on any item's state change causes O(N) re-rendering of all list items. This becomes noticeably slow for long lists.
**Action:** Extract list items into a `memo`-ized component. Since unchanged Map values maintain stable object references, `memo` correctly skips re-rendering unchanged list items, turning O(N) renders into O(1) for frequent single-item updates.
