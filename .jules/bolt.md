...
## 2024-11-21 - [Memoize List Items Driven by Map Derived State]
**Learning:** When driving a large list view directly from an frequently updated `Map` (like a batch status tracker), simple component extraction without memoization still results in O(N) re-renders, as the parent component must clone the Map to trigger an update, creating a new referential object which forces all unmemoized children to render.
**Action:** Extract list items that depend on individual Map entries into `React.memo` components. Pass the specific Map value directly via `map.get(index)`. The stable object reference of unmodified map values will allow `React.memo` to skip re-rendering for all unmodified items, reducing O(N) renders to O(1) changed item renders.
