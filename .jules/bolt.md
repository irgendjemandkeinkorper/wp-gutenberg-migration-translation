...
## 2026-08-18 - [DOM NodeList QuerySelectorAll Array Allocation]
**Learning:** Using `Array.from()` to convert the `NodeList` returned by `querySelectorAll("*")` into an array before iterating causes measurable unnecessary array allocation and processing overhead, particularly when dealing with large HTML fragments in the validator.
**Action:** When iterating over a static `NodeList` returned by `querySelectorAll`, prefer a standard `for` loop with index access and the `.length` property instead of `Array.from()` to eliminate the intermediate array allocation while preserving exact behavior.
