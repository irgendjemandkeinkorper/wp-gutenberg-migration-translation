...
## 2024-11-20 - [DOM NodeList Iteration and Array Allocation Avoidance]
**Learning:** Using `Array.from()` to convert a `NodeList` (such as the result of `querySelectorAll` or `children`) into an array before iterating causes unnecessary array allocation in memory. While not a massive bottleneck individually, in tight loops or recursive calls over large DOM structures (like `validateFragment`), this overhead adds up significantly, taxing memory GC and parsing speed. Furthermore, using `firstElementChild` and `nextElementSibling` allows you to safely iterate through a DOM structure while allowing for nodes to be modified/removed inline, without creating an intermediate snapshot array.
**Action:** When iterating over a `NodeList` or children, drop `Array.from()`. Instead, iterate directly using a standard `for` loop with an index and the `.length` property for static NodeLists like `querySelectorAll`, or use `firstElementChild`/`nextElementSibling` and `firstChild`/`nextSibling` properties for DOM trees where nodes might be mutated inline.
