...
## 2024-11-21 - [Extract Components from Lists for Map Value Props]
**Learning:** React components that render large lists based on Maps, where map elements are accessed by key via `map.get(index)`, can suffer from O(N) re-renders when the Map reference updates (even if the values of most items did not change).
**Action:** Extract list items into a `React.memo` component and pass the specific Map value (e.g., `batchStatus.get(index)`) as a prop. This leverages the stable object references of the Map values to skip unnecessary re-renders of the entire list.
