...
## 2024-11-21 - [List React Re-render Bottleneck]
**Learning:** In a React application, mapping over a large array and rendering inline components (like `<li>`) directly inside the `map` function causes the entire list to re-render whenever the parent component updates. If the state updates frequently (e.g., during a batch processing crawl where each item's status updates sequentially), this results in severe O(N) rendering overhead and UI stuttering.
**Action:** Extract list items into a separate component wrapped in `React.memo`. Pass down only the specific state needed for that item as props. This allows React to skip re-rendering the unchanged items, turning an O(N) render into an O(1) render for individual status updates.
