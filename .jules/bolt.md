...
## 2024-05-19 - Avoid O(N) re-renders for React list items by extracting and memoizing item component

**Learning:** When rendering a large list of items (e.g. `batch.map`) where each item derives state from a continuously updating map (e.g., `batchStatus.get(index)`), defining the item inline inside the `.map` will cause the entire list to re-render whenever *any* item's status updates. This is particularly problematic for UI components like `SourceInputPanel` during batch operations, as O(N) re-renders can freeze the UI.
**Action:** Extract list items into a separate component wrapped in `React.memo`, passing only the specific item data (e.g., `page`) and its targeted state (`batchStatus.get(index)`). This ensures only the single item that received an update will re-render, reducing the operation from O(N) to O(1) per update.
