...
**Learning:** During batch processing with frequent map updates (e.g. tracking progress in `batchStatus`), directly rendering every list item inside `batch.map` causes the entire list to re-render when a single item's status changes. This leads to $O(N^2)$ rendering overhead and noticeable interface lag on large lists.
**Action:** Always extract individual list items into a `React.memo` component when the list length is large and state updates frequently. Pass specific object slices (like a specific map value `batchStatus.get(index)`) so the unchanged object references automatically bypass `React.memo` and reduce rendering overhead to $O(1)$ per state update.
