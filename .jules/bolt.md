...
## $(date +%Y-%m-%d) - Prevent O(N) Re-renders with Memoized List Items
**Learning:** In React, rendering an array of items via `map` with complex inline components causes the entire list to re-render whenever the parent's state (like `batchStatus`) changes, even if the change only affects a single item. This creates an O(N) rendering bottleneck for frequent updates.
**Action:** Always extract complex list items into separate components wrapped in `React.memo`, passing only the specific state they need (e.g., `batchStatus.get(index)`) to ensure O(1) rendering updates.
