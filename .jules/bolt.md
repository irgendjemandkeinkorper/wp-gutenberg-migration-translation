...
## 2026-08-21 - [DOM NodeList Iteration and Array Allocation Avoidance]
**Learning:** Using `Array.from()` to convert a static `NodeList` (like the one returned by `querySelectorAll`) into an array before iterating causes unnecessary array allocations in memory. This adds measurable overhead, especially when parsing large DOM structures repeatedly.
**Action:** When iterating over a static `NodeList` like those returned by `querySelectorAll`, avoid `Array.from()`. Instead, iterate directly over the list using a standard `for` loop with an index and the `.length` property to eliminate the intermediate array allocation.
