...
## 2024-05-20 - Unsanitized Href in HTML Compiler Core
**Vulnerability:** Cross-Site Scripting (XSS) via unvalidated `href` attributes in the Core Gutenberg Compiler output (`src/lib/compiler/core.ts`).
**Learning:** While the system utilized an allowlist for tag attributes in `safeAttributes` (only allowing `href`, `title`, `target`, `rel`), the values of those attributes were inserted into the output markup without scheme validation. This allowed dangerous URIs like `javascript:` to be compiled into the final output. The validation logic was present in `validate.ts` but wasn't being used in the core compiler.
**Prevention:** Never assume an attribute value is safe just because the attribute name is whitelisted. Always apply specific validation based on the attribute's semantic meaning (e.g., using `isSafeUrl` for all `href` or `src` attributes) before writing it to a serialized HTML string.
