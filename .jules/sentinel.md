...
## 2024-05-20 - Unsanitized Href Bypass in Core Compiler
**Vulnerability:** Cross-Site Scripting (XSS) via unsafe protocols (`javascript:`) in `href` attributes during block compilation.
**Learning:** Although the tokenizer sanitizes initial HTML passes, the core compiler builds `a` tags explicitly and blindly passed through whatever `href` string was left in the SemanticNode representation. This meant dangerous payloads could still end up in the final Gutenberg block output.
**Prevention:** Always validate URLs specifically with an `isSafeUrl` check at the final string-concatenation layer. Enforce attribute security right before the `href` attribute is injected into the compiled markup string.
