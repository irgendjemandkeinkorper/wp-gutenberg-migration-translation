...
## 2025-10-27 - URL Validation Bypass via Parser Differential
**Vulnerability:** XSS filter bypass in `safeUrl` (in `src/lib/compiler/safe-content.ts`) due to Node.js `URL` constructor parser differential.
**Learning:** Browsers ignore whitespace and control characters in HTML attributes, but standard URL parsers (like the Node.js `URL` constructor) often treat strings with embedded control characters (e.g., `java\nscript:`) as relative paths (defaulting to the base URL's protocol, e.g., `https:`). This causes sanitizers to incorrectly classify malicious payloads as safe.
**Prevention:** Always strip whitespace and C0/C1 control characters (`/[\x00-\x20\x7F-\x9F]/g`) from the URL string *before* passing it to `new URL()` or performing protocol validation.
