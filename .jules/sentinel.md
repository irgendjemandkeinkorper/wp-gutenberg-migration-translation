...
## 2025-02-23 - Unsanitized Href in Gutenberg Inline Compiler
**Vulnerability:** Cross-Site Scripting (XSS) via `javascript:`, `vbscript:`, and `data:` URIs in `href` and `src` attributes of inline links generated by the core Gutenberg compiler.
**Learning:** While the core compiler strictly controls allowed tags and standard attributes, it blindly passes through the raw string values for allowed attributes (like `href`). This allows an attacker who controls the source content to inject malicious URIs (e.g., `javascript:alert(1)`) into the generated HTML.
**Prevention:** Always explicitly validate URL attributes (like `href` and `src`) against a strict URI checker (like `isSafeUrl`) during compilation or serialization, even if the attribute name itself is on an allowlist.
