...
## 2025-02-23 - URL Parser Differential leading to XSS Bypass
**Vulnerability:** XSS bypass via URL parser differential when using Node.js `new URL()` to validate `href` / `src` attributes. Node's `new URL("java\nscript:alert(1)", "https://example.com")` parses obfuscated URIs containing whitespace as relative paths, defaulting to the base `https:` protocol and successfully bypassing allowed protocol checks, whereas browsers will execute the `javascript:` payload.
**Learning:** You cannot rely on server-side or non-browser standard library parsers (like Node.js `URL` constructor) to perfectly emulate how a browser will interpret malformed or obfuscated URLs. Control characters and whitespace are stripped by browsers *before* scheme resolution, while Node.js might treat them as path segments.
**Prevention:** Always strip C0/C1 control characters and all whitespace (e.g. `val.replace(/[\x00-\x20\x7F-\x9F]/g, "")`) from the raw string *before* passing it to `new URL()` for protocol or host validation.
