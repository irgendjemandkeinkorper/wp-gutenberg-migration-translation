...
## 2025-02-23 - XSS Vulnerability in Link Compilation
**Vulnerability:** Cross-Site Scripting (XSS) via unsafe `href` attributes (e.g., `javascript:alert(1)`) during core semantic node compilation.
**Learning:** Although `compileCoreNode` enforce a strict allowlist of tag attributes (like `href`, `title`, `target`, `rel`), it failed to inspect the actual value of the `href` attribute. This allowed malicious payloads stored in an Intermediate Representation (IR) node to bypass sanitization and be serialized directly into Gutenberg HTML blocks.
**Prevention:** Always validate the content of URL-based attributes using a robust URL validator (like `isSafeUrl`) before serializing them into markup, even when the attribute name itself is explicitly allowlisted.
