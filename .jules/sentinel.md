...
## 2024-03-22 - XSS Bypass via Null Byte Parsing Differential
**Vulnerability:** The `isSafeUrl` function failed to block malicious payload protocols (e.g. `javascript:alert(1)`) if they contained a null byte (`\x00`), because `DOMParser` implicitly converts these characters into the Unicode replacement character (`\uFFFD`). This bypassed the strict `startsWith` string check.
**Learning:** Checking protocol safety using strict string matching alone is insufficient if the string is sanitized with an incomplete character set and will later be run through `DOMParser`, which performs implicit character replacements that alter the payload's prefix.
**Prevention:** Always strip the Unicode replacement character (`\uFFFD`) alongside whitespace and control characters when sanitizing URLs to ensure parity between how validation interprets the string and how the browser executes it.
