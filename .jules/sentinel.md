...
## 2024-05-18 - Node.js URL Parser Differential Fix
**Vulnerability:** XSS via obfuscated `javascript:` payloads containing control characters or whitespace.
**Learning:** Node.js `URL` constructor parses invalid schemes with spaces (e.g., `java script:alert(1)`) as relative URLs, allowing them to bypass scheme validation when the browser (which ignores the spaces) interprets them as malicious scripts.
**Prevention:** When validating URLs to prevent XSS, always strip all whitespace characters and control characters (`/[\x00-\x20\x7F-\x9F]/g`) *before* passing the string to `new URL()`.
