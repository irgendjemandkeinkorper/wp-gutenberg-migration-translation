...
## 2025-02-28 - Prevent XSS in Core Compiler Href Attributes
**Vulnerability:** The core Gutenberg inline compiler (`compileCoreNode` -> `safeAttributes`) used an allowlist for attributes (e.g. `href`, `title`, `target`, `rel`), but did not validate the content of the `href` attribute itself. This allowed `javascript:`, `vbscript:`, and malicious `data:` URIs to be passed directly through to the output, creating a Cross-Site Scripting (XSS) vector.
**Learning:** An attribute allowlist prevents arbitrary injection (like `onclick`) but is insufficient to protect URI attributes like `href` or `src`. The validation step existed in the HTML extraction layer (`isSafeUrl`), but was missing from the IR serialization layer.
**Prevention:** Always combine attribute allowlists with content validation (e.g., protocol checking) for any attribute that resolves to a URL or URI when serializing data out of internal representations.
