...
**Prevention:** Keep sensitive keys/credentials strictly in-memory (React/session state) and automatically evict legacy stored credentials on mount. Implement a secure Production Proxy Mode that utilizes a server-side backend to hold and attach private keys safely, keeping them completely hidden from frontend clients.
...
## 2025-02-23 - Data URI Allowlist Security Fix
...
## 2024-05-24 - Unsafe URLs not fully filtered in URL parser or attributes

**Vulnerability:** Unicode replacement character `\uFFFD` in URLs can obfuscate XSS vectors (e.g. `java\uFFFDscript:`). When parsed by `DOMParser`, null characters `\x00` are mapped to `\uFFFD` and bypass `.replace(/[\x00-\x20\x7F-\x9F]/g, "")`. Additionally, URL attributes parsed from semantic nodes like `href` inside `safeAttributes` were not being sanitized through `isSafeUrl`, allowing malicious inline attributes to survive Gutenberg block serialization.
**Learning:** URL attributes from intermediate parsed forms (like ASTs or semantic nodes) must be re-validated right before serialization because intermediate parsers might strip or transform characters, bypassing earlier normalizations. Also, JavaScript's regex replacing control characters must explicitly handle `\uFFFD` because of HTML parser behavior on null bytes.
**Prevention:** Include `\uFFFD` in the control characters blocklist for URL sanitization. Always call `isSafeUrl` explicitly on all serialized attributes that can contain paths/URIs (like `href`, `src`) during compilation step.
