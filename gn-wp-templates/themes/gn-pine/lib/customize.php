 ...
 * GolfNow - Pine.
 *
 * This file adds the Customizer additions to the GolfNow - Pine Theme.
 *
...
add_action( 'customize_register', 'golfnow_pine_customizer_register' );
/**
 ...
 */
function golfnow_pine_customizer_register( $wp_customize ) {

    $appearance = genesis_get_config( 'appearance' );

    $wp_customize->add_setting(
        'pine_default_header_image',
        [
    ...
    $wp_customize->add_control(
        new WP_Customize_Media_Control(
            ...
            'pine_default_header_image',
            [
                'label'       => __( 'Header Image', 'gn-pine' ),
                'settings'    => 'pine_default_header_image',
                'section'     => 'colors',
            ]
        )
    );
    ...
    $wp_customize->add_setting(
        ...
        [
            ...
            'validate_callback' => 'golfnow_pine_validate_font_selection',
        ]
    );
    ...
    $wp_customize->add_control(
        'theme_appearance_font_selection',
        [
            'description' => __( 'The font used for headers, navigation, and buttons', 'gn-pine' ),
            'label'       => __( 'Header Font', 'gn-pine' ),
            'type'        => 'select',
            'section'     => 'title_tagline',
            'priority'    => 11,
            'settings'    => 'theme_appearance_font_selection',
            'choices'     => golfnow_pine_available_fonts(),
        ]
    );
    ...
    foreach( $appearance['color_selection_keys'] as $color_selection ) {
        ...
        $selection_name = __( ucwords( str_replace( '-', ' ', $color_selection ) ), 'gn-pine' );

        $wp_customize->add_setting(
            'theme_appearance_' . $selection_id,
            [
                'default'           => $appearance[$color_selection],
                'validate_callback' => 'golfnow_pine_validate_color_selection',
            ]
        );
        ...
        $wp_customize->add_control(
            ...
            [
                ...
                'choices'     => golfnow_pine_available_color_selections(),
            ]
        );
    }
 ...
 */
function golfnow_pine_validate_logo_width( $validity, $width ) {

    if ( empty( $width ) || ! is_numeric( $width ) ) {
        $validity->add( 'required', __( 'You must supply a valid number.', 'gn-pine' ) );
    } elseif ( $width < 100 ) {
        $validity->add( 'logo_too_small', __( 'The logo width cannot be less than 100.', 'gn-pine' ) );
    }
 ...
 */
function golfnow_pine_validate_font_selection( $validity, $selection ) {

    $available_fonts = golfnow_pine_available_fonts();

    if ( empty( $selection ) || ! array_key_exists( $selection, $available_fonts ) ) {
        $validity->add( 'golfnow_pine_theme_font_mismatch', __( 'You must select from available fonts', 'gn-pine' ) );
    }
 ...
 */
function golfnow_pine_available_fonts() {
    $appearance = genesis_get_config( 'appearance' );
 ...
 */
function golfnow_pine_validate_color_selection( $validity, $selection ) {

    $available_color_selections = golfnow_pine_available_color_selections();

    if ( empty( $selection ) || ! array_key_exists( $selection, $available_color_selections ) ) {
        $validity->add( 'golfnow_pine_theme_color_selection_mismatch', __( 'You must select from available colors', 'gn-pine' ) );
    }
 ...
 * and we'd just have to enter it into the appearance config file.
 */
function golfnow_pine_available_color_selections() {
    $appearance = genesis_get_config( 'appearance' );
