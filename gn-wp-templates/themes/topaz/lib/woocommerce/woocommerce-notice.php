...
add_action( 'admin_print_styles', 'topaz_remove_woocommerce_notice' );
/**
 * Removes the default WooCommerce Notice.
 *
 * @since 2.3.0
 */
function topaz_remove_woocommerce_notice() {
...
add_action( 'admin_notices', 'topaz_woocommerce_theme_notice' );
/**
 ...
 * @since 2.3.0
 */
function topaz_woocommerce_theme_notice() {
	...
	if ( get_user_option( 'topaz_woocommerce_message_dismissed', get_current_user_id() ) ) {
		return;
	}
...
add_action( 'wp_ajax_topaz_dismiss_woocommerce_notice', 'topaz_dismiss_woocommerce_notice' );
/**
 * Adds option to dismiss Genesis Connect for Woocommerce plugin install prompt.
 *
 * @since 2.3.0
 */
function topaz_dismiss_woocommerce_notice() {

	update_user_option( get_current_user_id(), 'topaz_woocommerce_message_dismissed', 1 );

}

add_action( 'admin_enqueue_scripts', 'topaz_notice_script' );
/**
 * Enqueues script to clear the Genesis Connect for WooCommerce plugin install prompt on dismissal.
 *
 * @since 2.3.0
 */
function topaz_notice_script() {

	wp_enqueue_script( 'topaz_notice_script', get_stylesheet_directory_uri() . '/lib/woocommerce/js/notice-update.js', [ 'jquery' ], '1.0', true );

}

add_action( 'switch_theme', 'topaz_reset_woocommerce_notice', 10, 2 );
/**
 * Clears the Genesis Connect for WooCommerce plugin install prompt on theme change.
 *
 * @since 2.3.0
 */
function topaz_reset_woocommerce_notice() {

	global $wpdb;

	$args  = [
		'meta_key'   => $wpdb->prefix . 'topaz_woocommerce_message_dismissed', // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
		'meta_value' => 1, // phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_value
	];
	$users = get_users( $args );

	foreach ( $users as $user ) {
		delete_user_option( $user->ID, 'topaz_woocommerce_message_dismissed' );
	}

}

add_action( 'deactivated_plugin', 'topaz_reset_woocommerce_notice_on_deactivation', 10, 2 );
/**
 ...
 */
function topaz_reset_woocommerce_notice_on_deactivation( $plugin, $network_deactivating ) {
	...
	topaz_reset_woocommerce_notice();
