 ...
 * Sunstone Pro.
 *
 * This file adds the required WooCommerce setup functions to the Sunstone Pro Theme.
 *
...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'sunstone_pro_woocommerce_breakpoint' );
/**
 ...
 */
function sunstone_pro_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'sunstone_pro_default_products_per_page' );
/**
 ...
 */
function sunstone_pro_default_products_per_page() {

	return 8;

}

add_filter( 'woocommerce_pagination_args', 'sunstone_pro_woocommerce_pagination' );
/**
 ...
 */
function sunstone_pro_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'sunstone-pro' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'sunstone-pro' ) );

	return $args;

}

add_action( 'after_switch_theme', 'sunstone_pro_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 2.3.0
 */
function sunstone_pro_woocommerce_image_dimensions_after_theme_setup() {
	...
	sunstone_pro_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'sunstone_pro_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function sunstone_pro_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 2.3.0
 */
function sunstone_pro_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'sunstone_pro_gallery_image_thumbnail' );
/**
 ...
 */
function sunstone_pro_gallery_image_thumbnail( $size ) {
