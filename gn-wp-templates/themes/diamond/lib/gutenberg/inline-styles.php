...
add_action( 'wp_enqueue_scripts', 'diamond_custom_gutenberg_css' );
/**
 * Outputs front-end inline styles based on colors declared in config/appearance.php.
 *
 * @since 2.9.0
 */
function diamond_custom_gutenberg_css() {

	$appearance = genesis_get_config( 'appearance' );
	$font_selection = get_theme_mod( 'diamond_font_select', $appearance['fonts-url'] );
...
CSS;
	...
	$css .= diamond_inline_font_sizes();
	$css .= diamond_inline_color_palette();

	wp_add_inline_style( genesis_get_theme_handle() . '-gutenberg', $css );

}

add_action( 'enqueue_block_editor_assets', 'diamond_custom_gutenberg_admin_css' );
/**
 ...
 * @since 2.9.0
 */
function diamond_custom_gutenberg_admin_css() {
...
CSS;
	...
	$css .= diamond_editor_inline_color_palette();
