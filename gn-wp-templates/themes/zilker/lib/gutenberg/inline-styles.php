...
add_action( 'wp_enqueue_scripts', 'zilker_custom_gutenberg_css' );
/**
 * Outputs front-end inline styles based on colors declared in config/appearance.php.
 *
 * @since 2.9.0
 */
function zilker_custom_gutenberg_css() {

	$appearance = genesis_get_config( 'appearance' );
	$font_selection = get_theme_mod( 'zilker_font_select', $appearance['fonts-url'] );

	$css = <<<CSS

:root {
	--theme-font: 'Roboto', sans-serif;
	--theme-primary: {$appearance['primary-color']};
}
...
CSS;
	...
	$css .= zilker_inline_font_sizes();
	$css .= zilker_inline_color_palette();

	wp_add_inline_style( genesis_get_theme_handle() . '-gutenberg', $css );

}

add_action( 'enqueue_block_editor_assets', 'zilker_custom_gutenberg_admin_css' );
/**
 ...
 * @since 2.9.0
 */
function zilker_custom_gutenberg_admin_css() {
...
CSS;
	...
	$css .= zilker_editor_inline_color_palette();
 ...
 */
function zilker_inline_font_sizes() {
 ...
 */
function zilker_inline_color_palette() {
 ...
 * @return string The editor colors CSS if `editor-color-palette` theme support was declared.
 */
function zilker_editor_inline_color_palette() {
