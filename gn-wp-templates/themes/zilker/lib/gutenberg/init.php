...
add_action( 'wp_enqueue_scripts', 'zilker_enqueue_gutenberg_frontend_styles' );
/**
 * Enqueues Gutenberg front-end styles.
 *
 * @since 2.7.0
 */
function zilker_enqueue_gutenberg_frontend_styles() {
...
add_action( 'enqueue_block_editor_assets', 'zilker_block_editor_styles' );
/**
 * Enqueues Gutenberg admin editor fonts and styles.
 *
 * @since 2.7.0
 */
function zilker_block_editor_styles() {
	...
	if ( key_exists( 'fonts', $appearance ) && is_array( $appearance['fonts'] ) ) {
		...
		wp_enqueue_style(
			genesis_get_theme_handle() . '-gutenberg-theme-font',
	...
	wp_enqueue_style(
		genesis_get_theme_handle() . '-gutenberg-body-fonts',
...
add_filter( 'body_class', 'zilker_blocks_body_classes' );
/**
 ...
 */
function zilker_blocks_body_classes( $classes ) {
...
$zilker_appearance = genesis_get_config( 'appearance' );

// Adds support for editor font sizes.
add_theme_support(
	'editor-font-sizes',
	$zilker_appearance['editor-font-sizes']
);

// Adds support for editor color palette.
add_theme_support(
	'editor-color-palette',
	$zilker_appearance['editor-color-palette']
);

require_once get_stylesheet_directory() . '/lib/gutenberg/inline-styles.php';

add_action( 'after_setup_theme', 'zilker_content_width', 0 );
/**
 * Set content width to match the “wide” Gutenberg block width.
 */
function zilker_content_width() {

	$appearance = genesis_get_config( 'appearance' );

	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- See https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/924
	$GLOBALS['content_width'] = apply_filters( 'zilker_content_width', $appearance['content-width'] );
