...
function zilker_color_contrast( $color ) {
 ...
 */
function zilker_color_shade( $color, $percent ) {
 ...
 */
function zilker_color_rgb( $color, $transparency = 1 ) {
...
function zilker_color_gradient( $first_color, $second_color, $angle = 135 ) {
...
function zilker_video_is_video( $post_content ) {
    global $shortcode_tags;
    global $wp_embed; // Using wp_embed to cache videos

    // VIPBot doesn't like us overwriting the shortcode tags variable
    // Instead get_shortcode_regex() accepts an Array that does the same thing

    $wanted_shortcode_tags = array(
        'video' => $shortcode_tags['video'],
        'embed' => $shortcode_tags['embed']
    );
    
    // Get the absurd shortcode regexp.
    $video_regex = '#' . get_shortcode_regex( $wanted_shortcode_tags ) . '#i';
