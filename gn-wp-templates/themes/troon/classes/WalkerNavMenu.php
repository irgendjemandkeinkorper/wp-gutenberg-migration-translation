...
class Walker_Nav_Menu extends \Walker_Nav_Menu
{
    ...
    public function start_lvl(&$output, $depth = 0, $args = array())
    {
        ...
        if ($depth === 0) {
            $current_menu_first_level = wp_nav_menu(['menu' => $args->menu->slug, 'depth' => 1, 'echo' => 0]);
            preg_match_all(
                '/menu-item-([0-9]+)/',
                $current_menu_first_level,
                $top_count
            );

            $top_count_last_array = count($top_count) - 1;
            $top_count_last_item = count($top_count[$top_count_last_array]) - 1;
            $last_item_id = $top_count[$top_count_last_array][$top_count_last_item];

            if (str_replace(' ', '', $args->menu->name) . 'Dropdown' . $last_item_id === $parent_id) {
    ...
    public function start_el(&$output, $data_object, $depth = 0, $args = null, $current_object_id = 0)
    {
        ...
        $atts['class'] = array_filter($atts['class'], function ($value) {
        ...
        if (in_array('menu-item-has-children', $menu_item->classes, true)) {
            ...
            $atts['aria-label'] = apply_filters('the_title', $menu_item->title, $menu_item->ID) . ' Dropdown Menu';
            $atts['id'] = str_replace(' ', '', $args->menu->name) . 'Dropdown' . $menu_item->ID;
        }
